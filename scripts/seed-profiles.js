/**
 * Seed profiles for MatchSats — BNC 2026 edition
 * Based on real speakers and organizers from the Bitcoin Nairobi Conference 2026
 *
 * Usage (local):
 ...
 *
 * Custom event code:
 *   EVENT_CODE=BNC2026 node scripts/seed-profiles.js
 */

const Database = require("better-sqlite3");
const path = require("path");

const EVENT_CODE = process.env.EVENT_CODE || "BNC2026";
...
// Migration: add invite_code if missing
try { db.exec("ALTER TABLE profiles ADD COLUMN invite_code TEXT"); } catch {}

const now = Date.now();

// Real speakers + organizers from BNC 2026
const PROFILES = [
  {
  ...
  },
  {
    ...
    needs: "Energy economists, policy researchers, and journalists covering African development. Also Bitcoin mining operators.",
  ...
  },
  {
    ...
    location: "Nairobi, Kenya",
    core_vibe: "High Leverage",
    building: "Btrust — Bitcoin developer grants and education programs across Africa. Communications and storytelling for the African Bitcoin ecosystem.",
  ...
  },
  {
    ...
    location: "Kampala, Uganda",
    core_vibe: "Ship Mode",
    building: "Gorilla Sats — Bitcoin circular economy and merchant adoption network in Uganda. Also BNC Marketing lead.",
    needs: "Growth hackers and community managers for East African markets. Looking for merchant POS solutions.",
    vibe: "Marketing Bitcoin in Africa is different from anywhere else. Real stories, real people, real use cases — that's what works.",
  ...
  },
  {
    ...
    location: "London, UK",
    core_vibe: "High Leverage",
    building: "Bitcoin SheLeads — global network empowering women in Bitcoin. MBE awarded for services to financial inclusion.",
...
const seedAll = db.transaction(() => {
  for (const p of PROFILES) {
    ...
    console.log(`  ✓ ${p.name} — ${p.role} · ${p.location}`);
  }
});

seedAll();
console.log(`\nSeeded ${PROFILES.length} BNC 2026 profiles with event code "${EVENT_CODE}"`);
console.log("Done. Run this on Render shell to seed the live database.");
